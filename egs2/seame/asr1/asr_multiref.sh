...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asr_train_multiref \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/en_text,en_text,text" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/zh_text,zh_text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/en_text,en_text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/zh_text,zh_text,text" \
                --train_shape_file "${_logdir}/train.JOB.scp" \
        ...
        <"${asr_stats_dir}/train/text_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/train/en_text_shape.${token_type}"

        <"${asr_stats_dir}/train/text_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/train/zh_text_shape.${token_type}"
        ...
        <"${asr_stats_dir}/valid/text_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/valid/en_text_shape.${token_type}"

        <"${asr_stats_dir}/valid/text_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/valid/zh_text_shape.${token_type}"
    fi
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/en_text,en_text,text "
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/zh_text,zh_text,text "
            _opts+="--train_shape_file ${_split_dir}/speech_shape "
            _opts+="--train_shape_file ${_split_dir}/text_shape.${token_type} "
            _opts+="--train_shape_file ${_split_dir}/en_text_shape.${token_type} "
            _opts+="--train_shape_file ${_split_dir}/zh_text_shape.${token_type} "
            _opts+="--multiple_iterator true "
            ...
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/en_text,en_text,text "
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/zh_text,zh_text,text "
            _opts+="--train_shape_file ${asr_stats_dir}/train/speech_shape "
            _opts+="--train_shape_file ${asr_stats_dir}/train/text_shape.${token_type} "
            _opts+="--train_shape_file ${asr_stats_dir}/train/en_text_shape.${token_type} "
            _opts+="--train_shape_file ${asr_stats_dir}/train/zh_text_shape.${token_type} "
        fi
        ...
        ${python} -m espnet2.bin.launch \
            ...
            ${python} -m espnet2.bin.asr_train_multiref \
                ...
                --valid_shape_file "${asr_stats_dir}/valid/en_text_shape.${token_type}" \
                --valid_shape_file "${asr_stats_dir}/valid/zh_text_shape.${token_type}" \
                --resume true \
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        if "${use_k2}"; then
          ...
          if "${use_streaming}"; then
              ...
              asr_inference_tool="espnet2.bin.asr_inference_multiref"
          fi
        fi
