...
from espnet2.asr.espnet_model_cond3_multiref import ESPnetASRModelCond3MultiRef
...
class ASRTaskMultiRef(AbsTask):
    ...
    def build_preprocess_fn(
        ...
        if args.use_preprocessor:
            retval = MutliTokenizerCommonPreprocessor(
                train=train,
                token_type=[args.token_type, args.token_type, args.token_type],
                token_list=[args.token_list, args.token_list, args.token_list],
                bpemodel=[args.bpemodel, args.bpemodel, args.bpemodel],
                non_linguistic_symbols=args.non_linguistic_symbols,
                text_cleaner=args.cleaner,
                g2p_type=args.g2p,
                # NOTE(kamo): Check attribute existence for backward compatibility
                ...
                speech_name="speech",
                text_name=["text", "en_text", "zh_text"],
            )
        else:
            retval = None
        assert check_return_type(retval)
    ...
    @classmethod
    def build_model(cls, args: argparse.Namespace) -> ESPnetASRModelCond3MultiRef:
        assert check_argument_types()
        ...
        # 8. Build model
        model = ESPnetASRModelCond3MultiRef(
            vocab_size=vocab_size,
            frontend=frontend,
            specaug=specaug,
            normalize=normalize,
