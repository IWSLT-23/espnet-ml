...
debugmode=1
dumpdir=/scratch/swbd/   # directory to dump full features
...
# data
data_home=/project/ocean/byan/corpora/
swbd1_dir=$data_home/LDC/LDC97S62
eval2000_dir="$data_home/LDC/LDC2002S09/hub5e_00 $data_home/LDC/LDC2002T43"
rt03_dir=$data_home/LDC/LDC2007S10
# path to the Fisher corpus LDC2004T19 LDC2005T19 LDC2004S13 LDC2005S13 for LM training (optional)
fisher_dir="$data_home/LDC/LDC2004T19 $data_home/LDC/LDC2005T19 $data_home/LDC/LDC2004S13 $data_home/LDC/LDC2005S13"
...
if [ -z ${tag} ]; then
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_nbpe${nbpe}
    if ${do_delta}; then
    ...
    expname=${train_set}_${backend}_${tag}_nbpe${nbpe}
fi
expdir=exp/${expname}_${ngpu}gpu
mkdir -p ${expdir}
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --verbose ${verbose} \
        --resume ${resume} \
        --train-json ${feat_tr_dir}/data_${bpemode}${nbpe}.json.wrd \
        --valid-json ${feat_dt_dir}/data_${bpemode}${nbpe}.json.wrd
fi
