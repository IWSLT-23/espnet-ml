...
if [ $num_spkrs -eq 1 ]; then
  local/json2trn_onelang.py ${dir}/data.json ${dic} --num-spkrs ${num_spkrs} --refs ${dir}/ref.$lang.trn --hyps ${dir}/hyp.$lang.trn --lang $lang
  ...
  sclite -r ${dir}/ref.$lang.trn trn -h ${dir}/hyp.$lang.trn trn -i rm -o all stdout > ${dir}/result.$lang.txt

  echo "write a CER (or TER) result in ${dir}/result.$lang.txt"
  grep -e Avg -e SPKR -m 2 ${dir}/result.$lang.txt
  ...
  if ${wer}; then
    ...
    # make words for english portion only
    python local/format_to_mer.py --ref ${dir}/ref.$lang.trn --hyp ${dir}/hyp.$lang.trn
    sclite -r ${dir}/ref.$lang.trn.wrd trn -h ${dir}/hyp.$lang.trn.wrd trn -i rm -o all stdout > ${dir}/result.$lang.wrd.txt

    echo "write a WER result in ${dir}/result.$lang.wrd.txt"
    grep -e Avg -e SPKR -m 2 ${dir}/result.$lang.wrd.txt
  fi
