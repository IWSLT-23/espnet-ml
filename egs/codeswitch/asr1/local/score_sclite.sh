...
if [ $num_spkrs -eq 1 ]; then
  local/json2trn_nolid.py ${dir}/data.json ${dic} --num-spkrs ${num_spkrs} --refs ${dir}/ref.trn --hyps ${dir}/hyp.trn
  ...
  if ${wer}; then
    ...
    python local/format_to_mer.py --ref ${dir}/ref.trn --hyp ${dir}/hyp.trn
    sclite -r ${dir}/ref.trn.wrd trn -h ${dir}/hyp.trn.wrd trn -i rm -o all stdout > ${dir}/result.wrd.txt
