...
def get_parser():
    ...
    # 5000 is unk; above is zh and below is en
...
def main(args):
    ...
    convert(args.json, args.dict, args.refs, args.hyps, args.lang, args.num_spkrs)


def convert(jsonf, dic, refs, hyps, lang, num_spkrs=1):
    n_ref = len(refs)
    n_hyp = len(hyps)
    assert n_ref == n_hyp
            ...
            if num_spkrs == 1:
                if lang == "en":
                    seq = [
                        ...
                        if (int(i) < 5000 and int(i) > 0)
                    ]
                else:
                    seq = [
                        char_list[int(i)]
                        for i in j["utts"][x]["output"][0]["rec_tokenid"].split()
                        if (int(i) > 5000 and int(i) < 11237)
                    ]
