...
class Reporter(chainer.Chain):
    ...
    def report(
        ...
        chainer.reporter.report({"loss_ctc": ctc_loss}, self)
        chainer.reporter.report({"cer": cer}, self)
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.fusion_type == "add":
            ...
            zh_confidence = zh_posteriors[:,:,-3] / zh_posteriors[:,:,-2]
            zh_hs_pad = zh_hs_pad * zh_confidence.unsqueeze(-1)
            en_confidence = en_posteriors[:,:,-2] / en_posteriors[:,:,-3]
            en_hs_pad = en_hs_pad * en_confidence.unsqueeze(-1)
            hs_pad = zh_hs_pad + en_hs_pad
        else:
            import pdb;pdb.set_trace()

        # post fusion ctc loss
        ctc_loss = self.ctc(hs_pad.view(batch_size, -1, self.cond_adim), hs_len, ys_pad)
        if not self.training and self.error_calculator_ctc is not None:
            ys_hat = self.ctc.argmax(hs_pad.view(batch_size, -1, self.cond_adim)).data
            cer_ctc = self.error_calculator_ctc(ys_hat.cpu(), ys_pad.cpu(), is_ctc=True)
        else:
            cer_ctc = None
        ...
        loss = (
            ...
            + ((self.cond_weight / 3) * ctc_loss)
            + ((self.cond_weight / 3) * zh_ctc_loss)
            + ((self.cond_weight / 3) * en_ctc_loss)
        )
        self.loss = loss
