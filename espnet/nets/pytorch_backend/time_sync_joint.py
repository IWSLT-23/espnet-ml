...
class TimeSyncBeamSearchJoint(torch.nn.Module):
    ...
    def __init__(
        ...
        beam_size: int,
        ctc: torch.nn.Module,
        en_ctc: torch.nn.Module,
        zh_ctc: torch.nn.Module,
        ...
        for l in hyps:
            ...
            for c in cands:
                if c == self.blank:
                    ...
                    new_hyps.add(l)
                elif c == self.sos:     # ctc should never proposes sos/eos, only happens due to duplicated use of the vocab unit in monolingual masking
                    continue
                else:
                    l_plus = l + (int(c),)
    ...
    def search(self, enc_output: torch.Tensor, en_enc_output: torch.Tensor, zh_enc_output: torch.Tensor) -> List[Hypothesis]:
        """
        Params:
        ...
        bi_lpz = self.ctc.softmax(enc_output) * self.mono_weight
        en_lpz = self.en_ctc.softmax(en_enc_output) * (1-self.mono_weight)/2
        en_lpz[:,:,0] = en_lpz[:,:,0]/2
        zh_lpz = self.zh_ctc.softmax(zh_enc_output) * (1-self.mono_weight)/2
        zh_lpz[:,:,0] = zh_lpz[:,:,0]/2
        lpz = bi_lpz + en_lpz + zh_lpz
        lpz = lpz.log()
        lpz = lpz.squeeze(0)
