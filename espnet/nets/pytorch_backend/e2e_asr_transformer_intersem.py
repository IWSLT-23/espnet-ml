...
# helper fxn for align viterbi path to gt; returns indices for scatter
def align_to_index(viterbi_path, ground_truth):
    i = 0
    align = [-1] * len(viterbi_path)
    counts = [0] * len(viterbi_path)
    leading_zeros = True
    cumm = 0  # ptr to start of count range
    for j in range(len(viterbi_path)):
        # max i is len(ground_truth)
        ...
        if viterbi_path[j] == 0 or viterbi_path[j] == -1:
            align[j] = i
        elif viterbi_path[j] != ground_truth[i]:
            counts[cumm:j] = (j-cumm) * [j-cumm]
            cumm = j
            i += 1
            align[j] = i
        elif j > 0 and i < len(ground_truth) and viterbi_path[j-1] == 0 \
          and ground_truth[i] == ground_truth[i+1]: # repeats
            counts[cumm:j] = (j-cumm) * [j-cumm]
            cumm = j
            i += 1
            align[j] = i
            ...
            align[j] = i
    counts[cumm:j+1] = (j-cumm+1) * [j-cumm+1]
    return torch.tensor(align), torch.tensor(counts)
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad, ys_bert_pad, spm2wrd, bert2wrd):
        """E2E forward.
        ...
        # compute token aligned hs using mean pooling
        B, T, H = hs_pad.shape
        _, L = ys_pad.shape
        ...
        # add dist to loss
        self.loss += spm2bert_dist

        loss_data = float(self.loss)
        spm2bert_data = float(spm2bert_dist)
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(
                loss_ctc_data, loss_att_data, self.acc, cer_ctc, cer, wer, loss_data, spm2bert_data
            )
        else:
    ...
    def calculate_all_attentions(self, xs_pad, ilens, ys_pad, ys_bert_pad, spm2wrd, bert2wrd):
        ...
        self.eval()
        with torch.no_grad():
            self.forward(xs_pad, ilens, ys_pad, ys_bert_pad, spm2wrd, bert2wrd)
        ret = dict()
    ...
    def calculate_all_ctc_probs(self, xs_pad, ilens, ys_pad, ys_bert_pad, spm2wrd, bert2wrd):
