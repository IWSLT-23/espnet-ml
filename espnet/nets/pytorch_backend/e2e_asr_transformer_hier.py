...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        if args.mtlalpha > 0.0:
            ...
            self.inter_ctc = torch.nn.ModuleList([CTC(
                o, args.adim, args.dropout_rate, ctc_type=args.ctc_type, reduce=True
            ) for o in inter_odim])
        else:
            self.ctc = None
    ...
    def forward(self, xs_pad, ilens, ys_pad, ys_int1_pad, ys_int2_pad):
        ...
        hs_pad, hs_mask, hs_intermediates = self.encoder(xs_pad, src_mask, ctc_softmax=[m.softmax for m in self.inter_ctc] if self.self_conditioning else None)
        self.hs_pad = hs_pad
        ...
        if self.mtlalpha == 0.0:
            ...
            if self.intermediate_ctc_weight > 0 and self.intermediate_ctc_layers:
                for i, hs_intermediate in enumerate(hs_intermediates):
                    ...
                    loss_inter = self.inter_ctc[i](
                        hs_intermediate.view(batch_size, -1, self.adim), hs_len, ys_int_pad[i]
                    )
