...
from espnet.nets.pytorch_backend.transformer.encoder_hier import Encoder
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        if args.mtlalpha > 0.0:
            ...
            accs = ['<US>', '<CHN>', '<PT>', '<ES>', '<JPN>', '<IND>', '<KR>', '<UK>', '<RU>', '<CA>']
            acc_ids = [258 + x for x in range(len(accs))]
            acc_dict = {k:v for k,v in zip(acc_ids, accs)}
            self.inter_ctc = torch.nn.ModuleDict({
                str(x) : CTC(inter_odim[0], args.adim, args.dropout_rate, ctc_type=args.ctc_type, reduce=True)
                for x in acc_ids
            })
        else:
            self.ctc = None
        ...
        self.encoder = Encoder(
            ...
            ctc_softmax=args.self_conditioning,
            conditioning_layer_dim=inter_odim,
        )
        if args.mtlalpha < 1:
    ...
    def forward(self, xs_pad, ilens, ys_pad, ys_int1_pad, ys_int2_pad=None):
        ...
        """
        cat = ys_int1_pad[:,0][0]
        for x in range(ys_int1_pad[:,0].shape[0]):
            if ys_int1_pad[:,0][x] != cat:
        ...
        hs_pad, hs_mask, hs_intermediates = self.encoder(xs_pad, src_mask, ctc_softmax=self.inter_ctc if self.self_conditioning else None)
        self.hs_pad = hs_pad
        ...
        if self.mtlalpha == 0.0:
            ...
            if self.intermediate_ctc_weight > 0 and self.intermediate_ctc_layers:
                for i, hs_intermediate in enumerate(hs_intermediates):
                    # assuming hs_intermediates and hs_pad has same length / padding
                    ys_int_pad_unmask = ys_int_pad[i]
                    ys_int_pad_mask = ys_int_pad[i].masked_fill(((ys_int_pad_unmask < 258) & (ys_int_pad_unmask >= 0)), int(cat))
                    for k in self.inter_ctc.keys():
                        loss_inter = 0.0
                        if k == cat:
                            alpha = 1.0
                            loss_k = self.inter_ctc[k](
                                hs_intermediate.view(batch_size, -1, self.adim), hs_len, ys_int_pad_unmask
                            )
                        else:
                            alpha = 1.0
                            loss_k = self.inter_ctc[k](
                                hs_intermediate.view(batch_size, -1, self.adim), hs_len, ys_int_pad_mask
                            )
                        loss_inter += loss_k * alpha
    ...
    def encode(self, x):
        ...
        enc_output, _, _ = self.encoder(x, None, ctc_softmax=self.inter_ctc if self.self_conditioning else None)
    ...
    def calculate_all_attentions(self, xs_pad, ilens, ys_pad, ys_int1_pad, ys_int2_pad=None):
        ...
        self.eval()
        with torch.no_grad():
            self.forward(xs_pad, ilens, ys_pad, ys_int1_pad, ys_int2_pad)
        ret = dict()
